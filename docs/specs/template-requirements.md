...
| G6 | The displayed version is the bare `AppName X.Y.Z` by default: the toolchain is configured so no build metadata (commit hash, date) is appended | `standard.md` Version Display; `cli.md` VERSIONOUT | `IncludeSourceRevisionInInformationalVersion=false` in `Directory.Build.props` |
...
| G8 | The distributable is as few files as the stack can produce — ideally one self-contained executable — without unpacking any part of itself into a shared location at run time, because nothing cleans such a location up. Where bundling a dependency would require run-time unpacking, the dependency is reduced instead; what remains ships alongside, and the packaging step ships the whole build output rather than a named artifact. Debug symbols are never part of it: since the packaging step names nothing, a symbol file a dependency brings with it is excluded while the build output is assembled, not worked around at packaging time | `dotnet.md` NATIVEDEP (stack form); generic: **this document** | `Directory.Build.props` fails the publish when `IncludeNativeLibrariesForSelfExtract` is enabled and drops every `.pdb` from the publish list; `Setup.iss` ships the publish directory by wildcard; whether a native dependency can be removed altogether is AUDIT (`dotnet.md` NATIVEDEP) |
...
| G10 | Where the stack's distribution mechanism installs over an existing installation or removes one, it handles the application being in use: the processes holding the installed files are detected, the user is told and offered a way to have them closed, and nothing is force-closed unless the user chose that. An unattended (silent) run closes them and continues, because stopping there fails a deployment with nothing on screen to explain it, and a detection that cannot run lets the operation proceed rather than blocking it. A stack whose distribution mechanism cannot detect this states so and the rule stays an AUDIT item | **this document**; `dotnet.md` RUNNINGAPP (stack form) | `Setup.iss` states `CloseApplications=yes` for the update path — Inno Setup's own Restart Manager support — and drives the Restart Manager from `InitializeUninstall` for the uninstall path, which Inno Setup does not cover; `AppMutex` is deliberately not declared, because it blocks before the Restart Manager stage and cannot be limited to uninstall |
...
- [ ] G8: fewest files, ideally one executable; no run-time self-unpacking into a shared location; the packaging step ships the whole build output
...
- [ ] G10: installing over and removing a running application is handled — detected, the user is offered a way to close it, nothing force-closed on the user's behalf
