...
| G8 | The distributable is as few files as the stack can produce — ideally one self-contained executable — without unpacking any part of itself into a shared location at run time, because nothing cleans such a location up. Where bundling a dependency would require run-time unpacking, the dependency is reduced instead; what remains ships alongside, and the packaging step ships the whole build output rather than a named artifact | `dotnet.md` NATIVEDEP (stack form); generic: **this document** | `Directory.Build.props` fails the publish when `IncludeNativeLibrariesForSelfExtract` is enabled; `Setup.iss` ships the publish directory by wildcard; whether a native dependency can be removed altogether is AUDIT (`dotnet.md` NATIVEDEP) |
